...
if (!process.argv[4]) {
  winston.warn('Delete photoset, photoset name is undefined');
}
...
async.waterfall([
  ...
  function(flickrApi, token, next) {
    winston.info('Delete photoset by name '+process.argv[4]);
    // Get all photosets
  ...
  function(flickrApi, photosets, next) {
    ...
    if (results.length === 0) {
      return next('Delete photoset by name \''+process.argv[4]+'\' , photoset doesn\'t exist');
    }
